@extends('layouts.approver')

@section('title', 'Rejected Requests')

@section('content')

<div class="pt-2">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Rejected Requests</h1>
        <p class="text-gray-500 text-sm mt-1">Requests rejected at your level — the requester has been informed</p>
    </div>

    @if(count($requests) === 0)
    <div class="bg-white border border-gray-200 rounded-2xl p-12 text-center shadow-sm">
        <div class="w-16 h-16 rounded-full bg-red-50 flex items-center justify-center mx-auto mb-4">
            <svg class="w-8 h-8 text-red-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
        </div>
        <p class="text-gray-500 font-medium">No rejected requests</p>
        <p class="text-gray-400 text-sm mt-1">Any requests you reject will appear here.</p>
    </div>
    @else
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50">
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Requester</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Service</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Rejected At</th>
                        <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Reason</th>
                        <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @foreach($requests as $r)
                    @php
                        $requesterName  = $r->name ?? $r->owner_name ?? '—';
                        $requesterEmail = $r->email ?? $r->institute_email ?? '—';
                        $typeColors = [
                            'VPN'             => 'bg-indigo-100 text-indigo-700',
                            'Internet Access' => 'bg-cyan-100 text-cyan-700',
                            'VM Request'      => 'bg-green-100 text-green-700',
                            'Web Hosting'     => 'bg-orange-100 text-orange-700',
                        ];
                        $tc = $typeColors[$r->_type] ?? 'bg-gray-100 text-gray-600';
                    @endphp
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-red-600 text-xs font-bold shrink-0">
                                    {{ strtoupper(substr($requesterName, 0, 1)) }}
                                </div>
                                <div>
                                    <p class="font-medium text-gray-900">{{ $requesterName }}</p>
                                    <p class="text-gray-400 text-xs">{{ $requesterEmail }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $tc }}">{{ $r->_type }}</span>
                        </td>
                        <td class="px-6 py-4 text-gray-500 text-xs">
                            {{ $r->rejected_at?->format('d M Y, h:i A') ?? '—' }}
                        </td>
                        <td class="px-6 py-4 text-gray-600 text-xs max-w-xs">
                            {{ Str::limit($r->rejection_reason ?? '—', 80) }}
                        </td>
                        <td class="px-6 py-4">
                            @php $rArr = array_merge($r->toArray(), ['_type' => $r->_type, '_model' => $r->_model]); @endphp
                            <div class="flex justify-end">
                                <button type="button"
                                        onclick="openDetail({{ json_encode($rArr) }})"
                                        class="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-lg transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                    View
                                </button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
    @endif
</div>

@include('approver._view_modal')

@endsection
